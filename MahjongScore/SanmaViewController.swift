...
class SanmaViewController: UIViewController, UITextFieldDelegate {
    ...
    //4人のプレイヤー点数の和のUILabel(合計105,000点なら正しい)
    ...
    @objc func done1() {
        ...
        let db: SanmaScoreDB = SanmaScoreDB()
        ...
        if(winnerNum != -1){
            if(pay >= 0){
                //正常な値の場合
                var scoreChange = [-pay, -pay, -pay]
                scoreChange[winnerNum] = pay * 2
                print(scoreChange)
    ...
    @objc func done2() {
        ...
        let pay_parent = db.table2_2[pickerView2.selectedRow(inComponent: 2)][pickerView2.selectedRow(inComponent: 3)]
        ...
        if(winnerNum != -1){
            if(parentNum != -1){
                if(winnerNum != parentNum){
                    if(pay_child >= 0){
                        //正常な値の場合
                        var scoreChange = [-pay_child, -pay_child, -pay_child]
                        scoreChange[parentNum] = -pay_parent
                        scoreChange[winnerNum] = pay_child + pay_parent
                        print(scoreChange)
                        showScoreUpdateCheckDialog(scoreChange: scoreChange)
    ...
    @IBAction func uma0_10(_ sender: Any) {
        showResultScore(x: 10)
    }
    @IBAction func uma0_15(_ sender: Any) {
        showResultScore(x: 15)
    }
    @IBAction func uma0_20(_ sender: Any) {
        showResultScore(x: 20)
    }
    
    //点数初期化ボタン
    @IBAction func initScore(_ sender: Any) {
        let newScores = [35000, 35000, 35000]
    ...
    func updateScoreSum(){
        let playerScore = getPlayerScores()
        let sum = playerScore[0] + playerScore[1] + playerScore[2]
    ...
    func showResultScore(x: Int){
        ...
        var dict = [String: Int]()
        for i in 0...2{
            dict[playerNames[i]] = (oldScores[i] - 35000)
        }
        ...
        showSimpleAlert(title: "最終スコア(0-\(x))",
    ...
    func getPlayerNames() -> [String]{
        return  UserDefaults.standard.stringArray(forKey: "playerName3") ?? ["player1", "player2", "player3"]
    }
    //UserDefaultsからプレイヤーのスコア配列を取得する関数
    func getPlayerScores() -> [Int]{
        return  UserDefaults.standard.array(forKey: "playerScore3") as? [Int] ?? [35000, 35000, 35000]
    }
    //UserDefaultsにプレイヤーの名前配列をセットする関数
    func setPlayerNames(names: [String]){
        UserDefaults.standard.set(names, forKey: "playerName3")
    }
    //UserDefaultsにプレイヤーのスコア配列をセットする関数
    func setPlayerScores(scores: [Int]){
        UserDefaults.standard.set(scores, forKey: "playerScore3")
    }
    
}


extension SanmaViewController : UIPickerViewDelegate, UIPickerViewDataSource {
    // ドラムロールの列数
